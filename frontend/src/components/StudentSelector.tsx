...
// // Simple helper function to get unique values
// const getUniqueOptions = (students: Student[], getKey: (s: Student) => number, getName: (s: Student) => string) => {
//   const map = new Map<number, string>();
//   students.forEach(s => map.set(getKey(s), getName(s)));
...
//     let schoolsToShow = students;
//     if (selectedDistrict) {
//       schoolsToShow = students.filter(s => s.districtId === selectedDistrict);
//     }
...
//       // District only
//       filteredByLocation = students.filter(s => s.districtId === selectedDistrict);
//     } else if (selectedSchool) {
//       // School only
//       filteredByLocation = students.filter(s => s.locationId === selectedSchool);
//     }
...
//     if (schoolId) {
//       // If selecting a school, clear grade and student
//       updateFilters({ 
//         school: schoolId, 
//         grade: null, 
//         student: null 
...
//   // Reusable dropdown component for ID-based options
//   const IdDropdown = ({ 
//     label, 
//     selectedId, 
//     placeholder, 
...
//   // Regular dropdown for grade (string-based)
//   const GradeDropdown = () => (
//     <div className="space-y-2">
//       <label className={`block text-base font-semibold ${
//         (!selectedDistrict && !selectedSchool) ? 'text-gray-400' : 'text-gray-700'
//       }`}>
//         Grade
...
//             } ${(!selectedDistrict && !selectedSchool) ? 'opacity-50 cursor-not-allowed' : ''}`}
//           >
...
//   //         <span className="ml-2 text-xs text-gray-500">(No students match current filters)</span>
//   //       )}
//   //     </label>
//   //     <DropdownMenu>
//   //       <DropdownMenuTrigger asChild>
...
//   //               <div className="flex items-center w-full">
//   //                 <span className="mr-2 w-4">
//   //                   {selectedStudent?.id === student.id && <Check className="h-4 w-4" />}
//   //                 </span>
...
//   // Student dropdown component - Modified to require District, School, and Grade
// const StudentDropdown = () => {
//   const isStudentDropdownDisabled = !selectedDistrict || !selectedSchool || !selectedGrade;
  
//   return (
//     <div className="space-y-2">
//       <label className={`block text-base font-semibold ${
//         isStudentDropdownDisabled ? 'text-gray-400' : 'text-gray-700'
//       }`}>
//         Student
//         {isStudentDropdownDisabled && (
//           <span className="ml-2 text-xs text-gray-500">(Select district, school, and grade first)</span>
//         )}
//       </label>
//       <DropdownMenu>
//         <DropdownMenuTrigger asChild>
...
//             className={`w-full justify-between text-base font-normal h-10 bg-white border-[#C0D5DE] border-[1.6px] ${
//               selectedStudent ? 'ring-2 ring-blue-200 border-blue-300' : ''
//             } ${isStudentDropdownDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
//           >
//             <span className="truncate">
//               {selectedStudent ? `${selectedStudent.id} (${selectedStudent.grade})` : "Select student"}
//             </span>
...
//           </Button>
//         </DropdownMenuTrigger>
//         {!isStudentDropdownDisabled && filteredStudents.length > 0 && (
//           <DropdownMenuContent className="max-h-60 overflow-y-auto w-full min-w-[280px]">
//             <DropdownMenuItem
//               onClick={() => updateFilters({ student: null })}
//               className="text-base py-2 cursor-pointer text-gray-500"
...
//             {selectedDistrict && (
//               <div>District: {getDistrictName(selectedDistrict)}</div>
//             )}
//             {selectedSchool && (
//               <div>School: {getSchoolName(selectedSchool)}</div>
//             )}
//             {selectedGrade && (
//               <div>Grade: {selectedGrade}</div>
//             )}
//             {selectedStudent && (
//               <div>Student: {selectedStudent.id} ({selectedStudent.grade})</div>
//             )}
//           </div>
//           <div className="mt-2 text-xs text-gray-500">
//             Showing data for {filteredStudents.length} student{filteredStudents.length !== 1 ? 's' : ''}
//           </div>
//         </div>
//       )}
//     </div>
...
interface Props {
  ...
  selectedStudent: Student | null
  onSelect: (s: Student | null) => void
  onFiltersChange: (filters: { district: number | null; school: number | null; grade: string | null; student: Student | null }) => void
  selectedDistrict: number | null
  selectedSchool: number | null
  selectedGrade: string | null
}

const gradeOrder = ["Pre-Kindergarten", "Kindergarten", "1st Grade", "2nd Grade", "3rd Grade", "4th Grade", "5th Grade", "6th Grade", "7th Grade", "8th Grade", "9th Grade", "10th Grade", "11th Grade", "12th Grade"]
...
export const StudentSelector: React.FC<Props> = ({ students, selectedStudent, onSelect, onFiltersChange, selectedDistrict, selectedSchool, selectedGrade }) => {
  const getDistrictName = (id: number) => students.find(s => s.districtId === id)?.districtName || "Unknown"
  const getSchoolName = (id: number) => students.find(s => s.locationId === id)?.schoolName || "Unknown"

  const { districtOptions, schoolOptions, availableGrades, filteredStudents, activeFiltersCount, currentApiLevel } = useMemo(() => {
    const allDistricts = getUniqueOptions(students, s => s.districtId, s => s.districtName)
    const schoolsToShow = selectedDistrict ? students.filter(s => s.districtId === selectedDistrict) : students
    const availableSchools = getUniqueOptions(schoolsToShow, s => s.locationId, s => s.schoolName)

    let filteredByLocation = students
    if (selectedDistrict && selectedSchool) filteredByLocation = students.filter(s => s.districtId === selectedDistrict && s.locationId === selectedSchool)
    else if (selectedDistrict) filteredByLocation = students.filter(s => s.districtId === selectedDistrict)
    else if (selectedSchool) filteredByLocation = students.filter(s => s.locationId === selectedSchool)

    const finalFilteredStudents = selectedGrade ? filteredByLocation.filter(s => s.grade === selectedGrade) : filteredByLocation
    const grades = Array.from(new Set(filteredByLocation.map(s => s.grade))).sort((a, b) => gradeOrder.indexOf(a) - gradeOrder.indexOf(b))

    let apiLevel = "None"
    if (selectedStudent) apiLevel = "Student"
    else if (selectedGrade && (selectedDistrict || selectedSchool)) apiLevel = "Grade"
